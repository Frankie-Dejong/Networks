...
class UpsampleBlock(Module):
    def __init__(self, with_conv, channels=None, mode="nearest"):
        super().__init__()
    ...
    def execute(self, x, time_embeddings):
        B, C, H, W = x.shape
        x = jit.nn.resize(x, size=[H * 2, W * 2], align_corners=True)
        if self.with_conv:
            x = self.conv(x)
...
class DownsampleBlock(Module):
    def __init__(self, with_conv, channels=None):
        ...
        self.with_conv = with_conv
        if self.with_conv:
            self.conv = nn.Conv(channels, channels, 3, 2, 1)
...
class ResNetBlock(Module):
    def __init__(self, in_channels, out_channels, embedding_dims, dropout):
        ...
        self.conv1 = nn.Conv(in_channels, out_channels, 3, 1, 1)
        ...
        self.conv_residual = nn.Conv(in_channels, out_channels, 3, 1, 1)
        
    def execute(self, x, timeEmbedding):
        B, C, H, W = x.shape
        h = x
...
class AttnBlock(Module):
    ...
    def execute(self, x: jit.Var):
        ...
        w = jit.linalg.einsum('bhwc,bHWc->bhwHW', q, k) / jit.sqrt(C)
        w = w.reshape((B, H, W, H * W))
        w = nn.softmax(w, dim=-1)
        w = w.reshape((B, H, W, H, W))
        x = jit.linalg.einsum('bhwHW,bHWc->bhwc', w, v)
        x = self.proj_out(x)
...
class BasicBlock(Module):
    ...
    def execute(self, x, time_embedding):
...
class Unet(Module):
    def __init__(self, 
        ...
        self.conv_in = nn.Conv(3, in_channels, 3, 1, 1)
        down_blocks = []
        mid_blocks = []
        up_blocks = []
        ...
        self.conv_out = nn.Conv(in_channels, out_channels, 3, 1, 1)        
    ...
    def execute(self, x, t):
        ...
        for block in self.up_blocks:
            if isinstance(block, BasicBlock):
                ...
                map_this, temb = h
                new_map = jit.concat((map_this, map_from_down), dim=1)
                h = (new_map, temb)
                h = block(*h)
            else:
                h = block(*h)
